...
Expected inputs (example):
  --fmri-dir data/raw/lemon/fmri_timeseries   # *.npy, shape [time, roi]
  --eeg-dir  data/raw/lemon/eeg_timeseries    # *.npy, shape [channels, time]
  Matching filenames by subject id prefix, e.g. sub-001.npy
"""
...
def main() -> int:
    ...
    p.add_argument("--out-dir", default="data/processed/lemon")
    p.add_argument("--manifest", default="manifests/lemon_manifest.csv")
    ...
    for fmri_file in sorted(fmri_dir.glob("*.npy")):
        ...
        ts = np.load(fmri_file)          # [time, roi]
        eeg = np.load(eeg_file)          # [ch, time]
